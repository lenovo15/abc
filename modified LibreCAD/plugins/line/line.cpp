/*****************************************************************************/
/*  line.cpp - plugin example for LibreCAD                                 */
/*                                                                           */
...
QString LC_line::name() const
 {
     return (tr("line plugin"));
 }

PluginCapabilities LC_line::getCapabilities() const
{
    PluginCapabilities pluginCapabilities;
    pluginCapabilities.menuEntryPoints
            << PluginMenuLocation("Help", tr("line plugin"));
    return pluginCapabilities;
}

void LC_line::execComm(Document_Interface *doc,
                             QWidget *parent, QString cmd)
{
    Q_UNUSED(doc);
    Q_UNUSED(cmd);
    lc_linedlg pdt(parent);
    int result =  pdt.exec();
...
/*****************************/
lc_linedlg::lc_linedlg(QWidget *parent) :  QDialog(parent)
{
...
bool lc_linedlg::failGUI(QString *msg)
{
...
void lc_linedlg::procesAction(Document_Interface *doc)
{
    Q_UNUSED(doc);
    QPointF start, end;
...
void lc_linedlg::checkAccept()
{

    errmsg.clear();
    if (failGUI(&errmsg)) {
        QMessageBox::critical ( this, "line plugin", errmsg );
        errmsg.clear();
        return;
    }
    accept();
...
lc_linedlg::~lc_linedlg()
{
}
void lc_linedlg::closeEvent(QCloseEvent *event)
 {
    writeSettings();
...
void lc_linedlg::readSettings()
 {
    QString str;
    QSettings settings(QSettings::IniFormat, QSettings::UserScope, "LibreCAD", "line_plugin");
...
Q_EXPORT_PLUGIN2(lc_line, LC_line);
#endif
